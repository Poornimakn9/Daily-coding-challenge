...
SELECT p.plot_name, SUM(i.water_amount_liters) AS total_water_liters FROM plots p
...
SELECT p.soil_type, p.plot_name, y.yield_kg AS highest_yield_kg
FROM plots p
JOIN yields y ON p.plot_id = y.plot_id
WHERE (p.soil_type, y.yield_kg) IN (
    SELECT 
        p2.soil_type,
        MAX(y2.yield_kg)
    FROM plots p2
    JOIN yields y2 ON p2.plot_id = y2.plot_id
    GROUP BY p2.soil_type
);
...
SELECT DATE_FORMAT(harvest_date, '%Y-%m') AS month, COUNT(*) AS total_harvests
FROM yields
...
FROM plots p JOIN yields y ON p.plot_id = y.plot_id JOIN irrigation_logs i ON p.plot_id = i.plot_id
WHERE 
    y.yield_kg < (
