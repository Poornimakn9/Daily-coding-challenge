...
select c.comment_id, c.comment_text, p.post_id, u.username
from Comments c
JOIN Users u ON c.user_id = u.user_id
JOIN Posts p ON c.post_id = p.post_id;
select u.user_id, u.username, COUNT(p.post_id) AS total_posts from Users u
JOIN Posts p ON u.user_id = p.user_id
GROUP BY u.user_id, u.username ORDER BY total_posts desc limit 3;
...
AND u.user_id != 3;
select distinct p.post_id, p.content from Posts p
...
select u.user_id, u.username, COUNT(DISTINCT l.like_id) + COUNT(DISTINCT c.comment_id) AS influence_score
FROM Users u
LEFT JOIN Posts p ON u.user_id = p.user_id
...
SELECT 
    ...
    COUNT(DISTINCT l1.like_id) AS total_likes_given,
    COUNT(DISTINCT l2.like_id) AS total_likes_received,
